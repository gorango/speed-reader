<template>
  <section>
    <div>
      <textarea v-if='!reading' tabindex="0" v-model='text'></textarea>
      <article v-if='reading'>
        <p v-for='(block, index) in blocks' v-bind:class='{ active: blockIndex === index }'>
          <span v-for='token in block' v-html='token.text' v-bind:class='{ active: token.active }'></span>
        </p>
      </article>
    </div>
    <reader></reader>
    <div class='max-width-1 mx-auto flex items-center justify-center'>
      <button class="small" v-on:click='prev()'>
        <i class="material-icons">skip_previous</i>
      </button>
      <div class='flex flex-column items-center'>
        <button class="small" v-on:click='speed("UP")'>
          <i class="material-icons">expand_less</i>
        </button>
        <button v-if='!reading' v-on:click='read()'>
          <i class="material-icons">play_arrow</i>
        </button>
        <button v-if='reading' v-on:click='pause'>
          <i class="material-icons">pause</i>
        </button>
        <button class="small" v-on:click='speed("DOWN")'>
          <i class="material-icons">expand_more</i>
        </button>
      </div>
      <button class="small" v-on:click='next()'>
...
export default {
  ...
  computed: {
    reading () { return this.$store.state.reading },
    progress () { return this.$store.state.progress },
    blockIndex () { return this.$store.state.blockIndex },
    blocks () {
      /**
       * Split text into an array of blocks (paragraphs),
       * each containing an array of tokens (words, spaces, and wraps)
       * Find active token to highlight
       */
      return this.text
        .split('\n')
        .filter(p => p)
        .map((block, blockIndex) => {
          let adjustTokenIndex = 0
          return block
            .match(/["«»“”()/–—]|\s|--+|\n+|[^\s"“«»”()/–—]+/g)
            .map((token, tokenIndex) => {
              const ignore = '«»""“”()[] ' // ignore wraps and spaces
              const text = token

              if (ignore.includes(token)) {
                adjustTokenIndex = adjustTokenIndex + 1
                return { text }
              } else {
                tokenIndex = tokenIndex - adjustTokenIndex
                const active =
                  this.$store.state.blockIndex === blockIndex &&
                  this.$store.state.tokenIndex === tokenIndex
                return { active, text }
              }
            })
        })
  ...
  methods: {
    read (e) {
      if (e) {
        if (e.ctrlKey) this.$store.dispatch('read', this.text)
      } else {
        this.$store.dispatch('read', this.text)
      }
    },
    play () { this.$store.dispatch('play') },
    pause () { this.$store.dispatch('pause') },
    next () { this.$store.dispatch('skip', 'NEXT') },
    prev () { this.$store.dispatch('skip', 'PREV') },
    control (e) {
      ...
      if (e.target !== textarea || e.ctrlKey && e.target.value === 13) {
        switch (e.which) {
          ...
          case 37: // left
            ...
            else { /* TODO: skip to index is buggy */ }
            break
          case 38: // up
    ...
    speed (direction) { this.$store.dispatch('speed', direction) }
  }
}
</script>
